...
from ..detr_utils.segmentation import dice_loss, sigmoid_focal_loss
...
class DETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, aux_loss=False, object_embedding_loss=False, obj_embedding_head=None):
        ...
        self.input_proj = nn.Conv1d(backbone.num_channels[-1], hidden_dim, kernel_size=1)
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        ...
        boxes = box_ops.box_cxw_to_xx(out_bbox)
        # and from relative [0, 1] to absolute [0, length] coordinates
        # l = target_sizes.unbind(1)
        # scale_fct = torch.stack([l, l], dim=1)
        scale_fct = target_sizes.expand(-1, 2)
