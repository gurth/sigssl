...
class RODEvaluator(object):
    def __init__(self, opt, iou_types):
        ...
        for iou_type in iou_types:
            ...
            if iou_type == "bbox":
                if opt.no_cls:
                    self.eval[iou_type].params.useCats = 0
    ...
    def update(self, predictions, targets):
        ...
        for iou_type in self.iou_types:
            ...
            if eval_instance.gt is None:
                eval_instance.gt = results['gt']
            else:
                eval_instance.gt.extend(results['gt'])

            eval_instance.params.imgIds = img_ids

            eval_instance.params.catIds = list(range(0, self.num_classes ))
    ...
    def synchronize_between_processes(self):
        for iou_type in self.iou_types:
            ...
            tmp_eval_img = np.concatenate(self.eval_imgs[iou_type], 2)
            self.create_common_eval(self.eval[iou_type], self.img_ids, tmp_eval_img)#self.eval_imgs[iou_type])
    ...
    def create_common_eval(self, eval_instance, img_ids, eval_imgs):
        ...
        eval_instance._paramsEval = copy.deepcopy(eval_instance.params)
